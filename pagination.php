...
<div class="pagination-wrapper <?php echo esc_attr($wrapper_class); ?>">
    ...
    <div id="pagination" class="section-inner" data-query-args="<?php echo esc_attr($json_query_args); ?>" data-pagination-type="<?php echo esc_attr($pagination_type); ?>" data-load-more-target=".load-more-target">
        ...
        ?>
            ...
            ?>
                <button id="load-more" class="d-no-js-none do-spot spot-fade-up">
                    <span class="load-text"><?php esc_html_e('Load More', 'mcgillmed_theme'); ?></span>
        ...
        $prev_link = get_previous_posts_link('<span class="arrow stroke-cc">' . mcgillmed_theme_get_theme_svg('ui', 'arrow-left', 96, 49) . '</span><span class="screen-reader-text">' . esc_html__('Previous Page', 'mcgillmed_theme') . '</span></span>', $query_args['max_num_pages']);
        $next_link = get_next_posts_link('<span class="screen-reader-text">' . esc_html__('Next Page', 'mcgillmed_theme') . '</span></span><span class="arrow stroke-cc">' . mcgillmed_theme_get_theme_svg('ui', 'arrow-right', 96, 49) . '</span>', $query_args['max_num_pages']);
