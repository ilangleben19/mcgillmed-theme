...
if (!function_exists('mcgillmed_theme_the_custom_logo')) :
    function mcgillmed_theme_the_custom_logo()
    {

        echo esc_html(mcgillmed_theme_get_custom_logo());
    }
endif;

if (!function_exists('mcgillmed_theme_get_custom_logo')) :
    function mcgillmed_theme_get_custom_logo()
    {
        ...
        $logo_dark_id     = get_theme_mod('mcgillmed_theme_dark_mode_logo', null);
        $retina_logo     = get_theme_mod('mcgillmed_theme_retina_logo', false);
...
if (!function_exists('mcgillmed_theme_the_fallback_image')) :
    function mcgillmed_theme_the_fallback_image()
    {

        echo mcgillmed_theme_get_fallback_image();
    }
endif;

if (!function_exists('mcgillmed_theme_get_fallback_image')) :
    function mcgillmed_theme_get_fallback_image()
    {

        // If a valid fallback image is set in the Customizer, return the markup for it.
        $fallback_image_id = get_theme_mod('mcgillmed_theme_fallback_image');
...
/*	-----------------------------------------------------------------------------------------------
	...
	Output and get the SVG markup for a icon in the Mcgillmed_Theme_SVG_Icons class.
...
if (!function_exists('mcgillmed_theme_the_theme_svg')) :
    function mcgillmed_theme_the_theme_svg($group, $icon, $width = 24, $height = 24)
    {
        echo mcgillmed_theme_get_theme_svg($group, $icon, $width, $height);
    }
endif;

if (!function_exists('mcgillmed_theme_get_theme_svg')) :
    function mcgillmed_theme_get_theme_svg($group, $icon, $width = 24, $height = 24)
    {
        return Mcgillmed_Theme_SVG_Icons::get_svg($group, $icon, $width, $height);
    }
endif;
...
function mcgillmed_theme_get_social_link_svg($uri, $width = 24, $height = 24)
{
    return Mcgillmed_Theme_SVG_Icons::get_social_link_svg($uri, $width, $height);
}
...
function mcgillmed_theme_nav_menu_social_icons($item_output, $item, $depth, $args)
{

    // Change SVG icon inside social menu if there is a supported URL.
    if ('social' === $args->theme_location) {
        $svg = mcgillmed_theme_get_social_link_svg($item->url, 24, 24);
        if (!empty($svg)) {
...
add_filter('walker_nav_menu_start_el', 'mcgillmed_theme_nav_menu_social_icons', 10, 4);
...
if (!function_exists('mcgillmed_theme_the_social_menu')) :
    function mcgillmed_theme_the_social_menu($args = array())
    {

        $social_args = mcgillmed_theme_get_social_menu_args($args);
...
if (!function_exists('mcgillmed_theme_get_social_menu_args')) :
    function mcgillmed_theme_get_social_menu_args($args = array())
    {
...
if (!function_exists('mcgillmed_theme_get_archive_columns_classes')) :
    function mcgillmed_theme_get_archive_columns_classes()
    {

        $classes = array();

        // Get the array holding all of the columns options.
        $archive_columns_options = Mcgillmed_Theme_Customizer::get_archive_columns_options();
        ...
        foreach ($archive_columns_options as $setting_name => $setting_data) {
            ...
            switch ($setting_name) {
                case 'mcgillmed_theme_post_grid_columns_mobile':
                    $classes['mobile'] = 'cols-' . (12 / $value);
                    break;

                case 'mcgillmed_theme_post_grid_columns_tablet_portrait':
                    $classes['tablet_portrait'] = 'cols-t-' . (12 / $value);
                    break;

                case 'mcgillmed_theme_post_grid_columns_tablet_landscape':
                    $classes['tablet_landscape'] = 'cols-tl-' . (12 / $value);
                    break;

                case 'mcgillmed_theme_post_grid_columns_desktop':
                    $classes['desktop'] = 'cols-d-' . (12 / $value);
                    break;

                case 'mcgillmed_theme_post_grid_columns_desktop_large':
        ...
        return apply_filters('mcgillmed_theme_archive_columns_classes', $classes);
    }
endif;
...
if (!function_exists('mcgillmed_theme_the_archive_filter')) :
    function mcgillmed_theme_the_archive_filter()
    {

        // Check if we're showing the filter
        if (!mcgillmed_theme_show_home_filter()) return;
        ...
        // Use the mcgillmed_theme_home_filter_get_terms_args filter to modify which taxonomy is used for the filtration.
        $terms = get_terms(apply_filters('mcgillmed_theme_home_filter_get_terms_args', array(
            'depth'        => 1,
        ...
        // Make the home URL filterable. If you change the taxonomy of the filtration with `mcgillmed_theme_home_filter_get_terms_args`,
        // you might want to filter this to make sure it points to the correct URL as well (or maybe remove it altogether).
        $home_url = apply_filters('mcgillmed_theme_filter_home_url', $home_url);
    ...
    ?>
        ...
        <div class="filter-wrapper i-a a-fade-up a-del-200">
            <ul class="filter-list reset-list-style">
                ...
                <?php if ($home_url) : ?>
                    <li class="filter-show-all"><a class="filter-link active" data-filter-post-type="<?php echo esc_attr($post_type); ?>" href="<?php echo esc_url($home_url); ?>"><?php esc_html_e('Show All', 'mcgillmed_theme'); ?></a></li>
                <?php endif; ?>
    ...
    add_action('mcgillmed_theme_archive_header_end', 'mcgillmed_theme_the_archive_filter');
endif;
...
/*	-----------------------------------------------------------------------------------------------
	...
	filtered with mcgillmed_theme_show_home_filter.
--------------------------------------------------------------------------------------------------- */

if (!function_exists('mcgillmed_theme_show_home_filter')) :
    function mcgillmed_theme_show_home_filter()
    {

        $paged = get_query_var('paged') ? get_query_var('paged') : (get_query_var('page') ? get_query_var('page') : 1);

        return apply_filters('mcgillmed_theme_show_home_filter', (is_home() || is_page_template('page-templates/template-portfolio.php') || is_post_type_archive('jetpack-portfolio')) && $paged == 1 && get_theme_mod('mcgillmed_theme_show_home_filter', true));
    }
endif;
...
/* ------------------------------------------------------------------------------------------------
   ...
   context param passed to mcgillmed_theme_get_post_meta() determines which Customizer setting to retrieve.
--------------------------------------------------------------------------------------------------- */

if (!function_exists('mcgillmed_theme_maybe_output_post_meta_archive')) :
    function mcgillmed_theme_maybe_output_post_meta_archive()
    {

        global $post;

        // Escaped in mcgillmed_theme_get_post_meta().
        echo mcgillmed_theme_get_post_meta($post->ID, 'archive');
    }
    add_action('mcgillmed_theme_preview_end', 'mcgillmed_theme_maybe_output_post_meta_archive');
endif;

/* 
 * The mcgillmed_theme_entry_footer_start is only hooked into if there's post meta to output.
 * This allows us to use the has_action() check before outputting the entry footer wrapper in content.php.
 */

if (!function_exists('mcgillmed_theme_register_maybe_output_post_meta_single')) :
    function mcgillmed_theme_register_maybe_output_post_meta_single()
    {

        global $post;

        if ($post && mcgillmed_theme_get_post_meta($post->ID, 'single')) {
            add_action('mcgillmed_theme_entry_footer_start', 'mcgillmed_theme_maybe_output_post_meta_single');
        }
    }
    add_action('wp_head', 'mcgillmed_theme_register_maybe_output_post_meta_single');
endif;

if (!function_exists('mcgillmed_theme_maybe_output_post_meta_single')) :
    function mcgillmed_theme_maybe_output_post_meta_single()
    {

        global $post;

        // Escaped in mcgillmed_theme_get_post_meta().
        echo mcgillmed_theme_get_post_meta($post->ID, 'single');
    }
endif;
...
if (!function_exists('mcgillmed_theme_get_post_meta')) :
    function mcgillmed_theme_get_post_meta($post_id, $context = 'archive')
    {
        ...
        $post_types_with_post_meta     = Mcgillmed_Theme_Customizer::get_post_types_with_post_meta();
        ...
        $theme_mod_name = 'mcgillmed_theme_post_meta_' . $post_type;
        ...
        if ($post_meta && !in_array('empty', $post_meta)) {
             ...
             * added with the mcgillmed_theme_post_meta_items filter will not be affected by this sorting, 
             ...
             * @param int 		$post_id 			ID of the post.
             */
            $post_meta_order = apply_filters('mcgillmed_theme_post_meta_order', $post_meta_order, $post_id);
        ...
        $post_meta = apply_filters('mcgillmed_theme_post_meta_items', $post_meta, $post_id);
        ...
        $post_meta_wrapper_classes     = apply_filters('mcgillmed_theme_post_meta_wrapper_classes', array('post-meta-wrapper'), $post_id, $post_meta);
        $post_meta_classes             = apply_filters('mcgillmed_theme_post_meta_classes', array('post-meta'), $post_id, $post_meta);
        ...
        // Enable the $mcgillmed_theme_has_meta variable to be modified in actions.
        global $mcgillmed_theme_has_meta;

        // Default it to false, to make sure we don't output an empty container.
        $mcgillmed_theme_has_meta = false;
            ...
            <ul class="<?php echo esc_attr($post_meta_classes_str); ?>">
                ...
                ?>
                            <li class="date">
                                <?php
                                if ($context == 'single') {
                                    printf(esc_html_x('Published %s', '%s = The date of the post', 'mcgillmed_theme'), $entry_time_str);
                                } else {
                                    echo $entry_time_str;
                                }
                                ?>
                        ...
                        ?>
                            <li class="author">
                                ...
                                printf(esc_html_x('By %s', '%s = author name', 'mcgillmed_theme'), '<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '">' . esc_html(get_the_author_meta('display_name')) . '</a>'); ?>
                            </li>
                        <?php
                            break;
                        ...
                        case 'categories':
                            ...
                            $category_taxonomy = apply_filters('mcgillmed_theme_post_meta_category_taxonomy', $category_taxonomy, $post_id);

                            if (!has_term('', $category_taxonomy, $post_id)) break;
                            $mcgillmed_theme_has_meta = true;
                            $prefix = ($context == 'single') ? esc_html__('Posted in', 'mcgillmed_theme') : esc_html__('In', 'mcgillmed_theme');
                        ?>
                            <li class="categories">
                        ...
                        ?>
                            <li class="edit">
                                <a href="<?php echo esc_url(get_edit_post_link()); ?>">
                                    <?php esc_html_e('Edit', 'mcgillmed_theme'); ?>
                                </a>
                            </li>
                        <?php
                            break;
                        ...
                        case 'tags':
                            ...
                            $tag_taxonomy = apply_filters('mcgillmed_theme_post_meta_tag_taxonomy', $tag_taxonomy, $post_id);
                        ...
                        ?>
                            <li class="tags">
                                ...
                                if ($tag_taxonomy == 'post_tag') {
                                    echo get_the_tag_list(esc_html__('Tagged', 'mcgillmed_theme') . ' ', ', ', '', $post_id);
                                } else {
                                    the_terms($post_id, $tag_taxonomy, esc_html__('Tagged', 'mcgillmed_theme') . ' ', ', ');
                                }
                                ?>
                            </li>
                <?php
                        ...
                        default:
                             ...
                             * any of the types supported out of the box in McGill Med Theme. If you've 
                             * added a custom post meta type in a child theme, you can output it 
                             * here by hooking into mcgillmed_theme_post_meta_[your-post-meta-key].
                             * 
                             * Note: If you add any output to this action, make sure you include 
                             * $mcgillmed_theme_has_meta as a global variable and set it to true. This will 
                             * tell mcgillmed_theme_get_post_meta() that there's post meta to output.
                             * 
                             * @param int 	$post_id 	ID of the post.
                             */

                            do_action('mcgillmed_theme_post_meta_' . $post_meta_item, $post_id);
                    } // End switch
...
<?php
        ...
        return ($mcgillmed_theme_has_meta && $meta_output) ? $meta_output : '';
    }
endif;
